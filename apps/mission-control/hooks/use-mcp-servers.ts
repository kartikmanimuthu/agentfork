...
async function fetchMcpServers(filters: McpServerFilters = {}): Promise<McpServerListResult> {
  ...
  const res = await fetch(`${BASE_PATH}/api/mcp-servers?${params}`);
  if (!res.ok) throw new Error('Failed to fetch MCP servers');
  return res.json();
}

async function fetchMcpServer(id: string): Promise<McpServer> {
  const res = await fetch(`${BASE_PATH}/api/mcp-servers/${id}`);
...
async function createMcpServer(input: {
  ...
  const res = await fetch(`${BASE_PATH}/api/mcp-servers`, {
    method: 'POST',
...
async function updateMcpServer(id: string, input: Partial<McpServer> & { changeNotes?: string }): Promise<McpServer> {
  const res = await fetch(`${BASE_PATH}/api/mcp-servers/${id}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(input),
...
async function deleteMcpServer(id: string): Promise<void> {
  const res = await fetch(`${BASE_PATH}/api/mcp-servers/${id}`, { method: 'DELETE' });
  if (!res.ok) throw new Error('Failed to delete MCP server');
}

async function testMcpServer(id: string): Promise<{ connected: boolean; toolCount?: number; tools?: string[]; error?: string }> {
  const res = await fetch(`${BASE_PATH}/api/mcp-servers/${id}/test`, { method: 'POST' });
