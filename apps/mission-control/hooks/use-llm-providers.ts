...
async function fetchLlmProviders(): Promise<LlmProvider[]> {
  const res = await fetch(`${BASE_PATH}/api/llm-providers`);
...
async function fetchLlmProvider(id: string, withSecrets = false): Promise<LlmProvider> {
  const res = await fetch(`${BASE_PATH}/api/llm-providers/${id}${withSecrets ? '?withSecrets=1' : ''}`);
...
async function updateLlmProvider(id: string, input: UpdateLlmProviderInput): Promise<LlmProvider> {
  const res = await fetch(`${BASE_PATH}/api/llm-providers/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(input),
  });
...
async function deleteLlmProvider(id: string): Promise<void> {
  const res = await fetch(`${BASE_PATH}/api/llm-providers/${id}`, { method: 'DELETE' });
...
async function setDefaultLlmProvider(id: string): Promise<LlmProvider> {
  const res = await fetch(`${BASE_PATH}/api/llm-providers/${id}/set-default`, { method: 'POST' });
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
...
async function validateProvider(input: ValidateProviderInput): Promise<ValidateProviderResponse> {
  const res = await fetch(`${BASE_PATH}/api/llm-providers/validate`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(input),
  });
...
async function refreshModels(id: string): Promise<LlmProvider> {
  const res = await fetch(`${BASE_PATH}/api/llm-providers/${id}/refresh-models`, { method: 'POST' });
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
...
export function useSetDefaultLlmProvider() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => setDefaultLlmProvider(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: llmProviderKeys.lists() }),
  });
}
...
export function useRefreshModels() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => refreshModels(id),
    onSuccess: (data) => {
