...
import { Plus, Settings, Trash2, Star, Cpu, RefreshCw } from 'lucide-react';
...
export default function LlmProvidersPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl flex-1 space-y-6 p-4 pt-6 md:p-8">
      ...
      <p className="text-muted-foreground">Manage LLM providers for Claw&apos;s chat inference and embeddings.</p>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between gap-4">
            ...
            <Link href="/llm-providers/new" className={buttonVariants()}>
              ...
              {providers?.map((provider) => (
                <div key={provider.id} className="flex items-center justify-between rounded-lg border p-4 hover:bg-accent/50 transition-colors">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <Link href={`/llm-providers/${provider.id}`} className={buttonVariants({ variant: 'ghost', size: 'icon', className: 'h-8 w-8' })} aria-label="Edit">
