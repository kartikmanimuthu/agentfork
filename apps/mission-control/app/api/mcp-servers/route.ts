...
import { getPrismaClient, createMcpServerSchema, createLogger } from '@chatbot/shared';
...
const logger = createLogger('mission-control:api:mcp-servers');
...
export async function GET(req: NextRequest) {
  try {
    ...
    const service = new McpServerService(session.studio.tenantId, db as any);
    const result = await service.findMany({ status: status as any, transport: transport as any, search, page, pageSize });
...
export async function POST(req: NextRequest) {
  try {
    ...
    await versionService.create(server.id, server.config as any, 'Initial version');
