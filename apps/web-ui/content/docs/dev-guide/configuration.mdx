...
> **Security:** Never commit `.env` files to version control. Use environment-specific secrets management (AWS Secrets Manager, Vault, etc.) in production.
...
|----------|---------|-------------|
| `WORKER_ARCH` | `vertical` | Execution strategy: `vertical` (single process) or `horizontal` (multi-process) |
| `WORKER_CONCURRENCY` | `5` | Number of concurrent jobs per worker |
| `WORKER_POLL_INTERVAL` | `2000` | Job polling interval in milliseconds |
