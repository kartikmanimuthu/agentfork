...
export class ContactLock {
  ...
  async acquire(accountId: string, chatId: string): Promise<boolean> {
    const key = `tg:lock:${accountId}:${chatId}`;
    return this.provider.acquire(key, this.ttlMs);
  }

  async release(accountId: string, chatId: string): Promise<void> {
    const key = `tg:lock:${accountId}:${chatId}`;
