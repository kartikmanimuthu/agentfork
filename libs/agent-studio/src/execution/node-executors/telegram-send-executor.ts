...
import type { TelegramSendNodeConfig } from '../../types/nodes';

const logger = createLogger('agent-studio:telegram-send-executor');

export class TelegramSendNodeExecutor implements NodeExecutor {
  type = 'telegram_send';

  async execute(ctx: NodeExecutionContext): Promise<NodeExecutionResult> {
    const config = ctx.config as TelegramSendNodeConfig;
    ...
    try {
      ...
      const replyToMessageId = config.replyToChannel
        ? channels[config.replyToChannel]
        : undefined;

      const response = await client.sendMessage(String(chatId), String(messageContent), {
        parseMode: config.parseMode,
        replyToMessageId: replyToMessageId ? parseInt(String(replyToMessageId), 10) : undefined,
      });

      const sentMessageId = String(response.message_id);

      logger.info({ nodeId: ctx.node.id, sentMessageId }, 'Telegram message sent');
      ...
      logger.error({ nodeId: ctx.node.id, error }, 'Telegram send failed');
      throw error;
    }
  }
}
